(() => {
  ...
  function renderQuestion() {
    ...
    const isLearning = true; // show right/wrong feedback for every question, in every quiz mode
    ...
    if (stored?.locked && isLearning) {
      ...
      if (!stored.verified) {
        ...
        feedback.textContent = stored.correct ? 'إجابة صحيحة، أحسنت.' : `إجابتك خاطئة. الإجابة الصحيحة هي: ${correct?.text || 'غير محددة'}`;
      }
  ...
  function chooseAnswer(q, answer) {
    ...
    state.answers[String(q.id)] = { answerId: String(answer.id), correct: verified ? !!answer.correct : null, verified, locked: true, sectionIndex: q.sectionIndex };
    renderQuestion();
    // Auto-advance to the next question only in learn/review mode, and only on a correct answer.
  ...
  // Probe a single candidate URL in the background (never touches the visible
  // <img>/<video> element) so several candidates can race in parallel instead
  // of being tried one-by-one — this is what used to make images feel stuck.
  function probeMedia(url, kind, timeoutMs) {
    return new Promise(resolve => {
      let done = false;
      const finish = ok => { if (done) return; done = true; clearTimeout(timer); resolve(ok ? url : null); };
  ...
  // Finds the first candidate URL that actually loads. All candidates
  // (local file, cached URL, guessed month URLs, and the WordPress media
  // API lookup) are raced in parallel with a short per-candidate timeout,
  // so the total wait is bounded by the slowest single attempt instead of
  // piling up one after another — this is the fix for images that used to
  // hang for a long time before showing up (or failing).
  async function findWorkingMediaUrl(media, kind = 'image') {
    const id = media?.mediaId;
    const cache = safeJSON(STORAGE.media, {});
    const candidates = new Set();
    if (media?.filename) candidates.add(`assets/questions/${media.filename.split('/').map(encodeURIComponent).join('/')}`);
    if (id && cache[id]) candidates.add(cache[id]);
    directMediaCandidates(media?.filename).forEach(u => candidates.add(u));

    const PER_TRY_MS = 3000;
    const probes = [...candidates].map(u => probeMedia(u, kind, PER_TRY_MS));
    ...
    if (id && !cache[id]) {
      const apiProbe = (async () => {
        try {
          ...
          if (!url) return null;
          cache[id] = url; saveJSON(STORAGE.media, cache);
          return probeMedia(url, kind, PER_TRY_MS);
        } catch { return null; }
      })();
      probes.push(apiProbe);
    }

    if (!probes.length) return null;
    return new Promise(resolve => {
      let remaining = probes.length, settled = false;
      probes.forEach(p => Promise.resolve(p).then(result => {
        if (settled) return;
        if (result) { settled = true; resolve(result); return; }
        remaining--;
  ...
  async function renderImage(q) {
    ...
    if (q.video?.filename) {
      wrap.hidden = false;
      if (!vid) { loader.textContent = q.video?.alt || 'تعذر تحميل فيديو السؤال'; return; }
      const url = await findWorkingMediaUrl(q.video, 'video');
      if (token !== state.imageToken) return;
      if (!url) { loader.hidden = false; loader.textContent = q.video?.alt || 'تعذر تحميل فيديو السؤال. تأكد من اتصال الإنترنت.'; vid.hidden = true; return; }
      vid.src = url; loader.hidden = true; vid.hidden = false;
      return;
    }
    ...
    const url = await findWorkingMediaUrl(q.image || {}, 'image');
    if (token !== state.imageToken) return;
    if (!url) { loader.hidden = false; loader.textContent = q.image?.alt || 'تعذر تحميل صورة السؤال. تأكد من اتصال الإنترنت.'; img.hidden = true; return; }
    img.src = url; loader.hidden = true; img.hidden = false;
  }
